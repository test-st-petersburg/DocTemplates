		...
		<xsl:element name="office:document-settings">
			...
			<xsl:element name="office:settings" inherit-namespaces="no">
				<xsl:merge>
					<xsl:merge-source
						for-each-item="
							$p:source-document/office:document-settings/office:settings,
							$p:embed-objects/office:document-settings/office:settings
						"
						select=" config:config-item-set "
						sort-before-merge="yes"
					>
						<xsl:merge-key select=" @config:name " order="ascending"/>
					</xsl:merge-source>
					<xsl:merge-action>
						<xsl:copy select=" head( current-merge-group() ) " inherit-namespaces="no" copy-namespaces="no">
							<xsl:copy-of select=" head( current-merge-group() )/@* "/>
							<xsl:merge>
								<xsl:merge-source for-each-item=" current-merge-group() "
									select=" config:config-item "
									sort-before-merge="yes"
								>
									<xsl:merge-key select=" @config:name " order="ascending"/>
								</xsl:merge-source>
								<xsl:merge-action>
									<xsl:copy select=" head( current-merge-group() ) " inherit-namespaces="no" copy-namespaces="no">
										<xsl:copy-of select=" head( current-merge-group() )/@* " copy-namespaces="no"/>
										<xsl:value-of select=" head( current-merge-group() ) "/>
									</xsl:copy>
								</xsl:merge-action>
							</xsl:merge>
						</xsl:copy>
